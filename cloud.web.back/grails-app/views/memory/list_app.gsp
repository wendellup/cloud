...
<head>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            $('#dg').datagrid({
                url: '${createLink(controller: 'memory',action: 'list',params:[objType:2])}',
                columns: [
                    [
                        {field: 'ck', title: '选择',checkbox:true, align: 'center'},
                        {field: 'id', title: 'ID', width: 60, align: 'center'},
                        {field: 'startTime', title: '推送开始时间', align:'center'},
                        {field: 'title', title: '资源应用标题', width: 150,align:'center'},
                        {field: 'description', title: '资源应用描述', width: 150,align:'center'},
                        {field: 'advertTypeName', title: '推送展现类型', width: 80,align:'center'},
                        {field: 'linkUrl', title: '资源应用代码', width: 120,align:'center'},
                        {field: 'gameName', title: '资源应用名称', width: 120,align:'center'},
                        ...
                        {field: 'gameSize', title: '应用大小', width: 60,align:'center'},
                        ...
                        {field: 'packageName', title: '应用包名',width: 120,align:'center'},
                        {field: 'realGameDownloadUrl', title: '资源应用包体',width: 120,align:'center'},
                        {field: 'ownName', title: '应用主名称',width: 120,align:'center'},
                        {field: 'pricingWay', title: '应用定价方式',width: 120,align:'center'},
                        {field: 'activationCondition', title: '应用激活条件',width: 120,align:'center'},
                        {field: 'recyclingPrice', title: '应用回收单价',width: 120,align:'center'}
                    ]
                ],
...
//                    iframeHeightAuto(window.parent.frames["iframe2"]);
                },
                height: 600,
                fitColumns: false,
                pagination: true,
        ...
        function toAdd(){
            location.href = "${createLink(controller: 'memory',action:'addMemory',params:[objType:2])}";
        }
        ...
        function AddMemory() {
            ...
            showDialog({
                ...
                height: 520,
                title: "新增常驻内存",
                url: "${createLink(controller: 'memory',action:'addMemory',params:[objType:2])}"
            });
        }
        ...
        function toEdit(){
            ...
            location.href = "${createLink(controller: 'memory',action:'editMemory')}?id=" + rows[0].id + "&objType=2";
        }
        ...
        function editMemory() {
            ...
            showDialog({
                ...
                height: 580,
                title: "编辑常驻内存",
                url: "${createLink(controller: 'memory',action:'editMemory')}?id=" + rows[0].id+ "&objType=2"
            });
        }
...
<e:layout fit="true">
    ...
    <h3 class="nopoint"><strong>推送管理&nbsp;&gt;&nbsp;应用推送</strong></h3>
    <e:form id="editForm">
