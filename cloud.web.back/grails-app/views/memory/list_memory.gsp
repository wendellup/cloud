...
<head>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            $('#dg').datagrid({
                url: '${createLink(controller: 'memory',action: 'list',params:[objType:1])}',
                columns: [
                    [
                        ...
                        {field: 'gameSize', title: '游戏大小', width: 60,align:'center'},
                        ...
                        {field: 'packageName', title: '游戏包名',width: 120,align:'center'},
...
//                    var selectedFrame = "iframe"+1;
//                    iframeHeightAuto(window.parent.frames[selectedFrame]);

//                    var temp = window.parent.document.getElementById('iframe_index').value;
//                    alert(temp);
//                    var selectedFrame = "iframe"+Number(temp+1);
//                    alert(selectedFrame);
//                    iframeHeightAuto(window.parent.frames[selectedFrame]);

//                    iframeHeightAuto(window.parent.frames["iframe"+(temp+1)]);
//                    var curTab = $('.ui-tabs-panel:not(.ui-tabs-hide)');
//                    var tabIndex = $("#tt > frame", window.parent).tabs('option', 'selected');
//                    var tab = $("#tt", window.parent).tabs('getSelected');
//                    var tabIndex = $("#tt", window.parent).tabs('getTabIndex',tab);
//                    alert(curTab);
//                    var curTabIndex = curTab.index();
//                    alert(tabIndex);

//                    var iframeTab = $("#tt", window.parent);//假设是tab
...
//                    $("iframe",tab).contents().find("input").val();获取tab中的iframe中的input的值
        ...
        function toAdd(){
            location.href = "${createLink(controller: 'memory',action:'addMemory',params:[objType:1])}";
        }
        ...
        function toEdit(){
            ...
            location.href = "${createLink(controller: 'memory',action:'editMemory')}?id=" + rows[0].id + "&objType=1";
        }
...
<e:layout fit="true">
    ...
    <h3 class="nopoint"><strong>推送管理&nbsp;&gt;&nbsp;游戏推送</strong></h3>
    <e:form id="editForm">
