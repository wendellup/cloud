...
<head>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            if ($('#pageTypeStr').val() == '1') {
                $("#tag_sortNo").show();
            }
            $('#dg').datagrid({
                url: '${createLink(controller: 'parameterTag',action: 'queryList')}',
...
//                    }
            ...
            $('#clientMenuTree').tree({
                onClick: function (node) {
                    ...
                    // 批量导入2013-09-27
                    %{--var span = $('#import_span');--}%
                    %{--span.empty();--}%
                    %{--span.append('<a href="javascript:void(0)" class="btn_orange" id="importGame">批量导入</a>');--}%
                    %{--var url = "${createLink(controller: 'game',action:'importGameWithColumn')}?pageType="+$('#pageTypeStr').val();--}%
                    %{--createUpload("importGame", {id:id,rename:true}, url, function(file, response){--}%
                        %{--if(response.result){--}%
                ...
                $('#dg').datagrid({
                    ...
                    columns: [
                        [
                            ...
                            {field: 'tagName', title: '标签名', width: 160, align:'center'},
                            {field: 'tagType', title: '标签类型', width: 160,align:'center'},
                            {field: 'remark', title: '标签描述', width: 160,align:'center'},
                        ]
                    ]
                });
            }
        ...
        function addTag() {
            ...
            showDialog({
                ...
                height: 300,
                title: "新增标签",
                url: "${createLink(controller: 'parameterTag',action:'addTag')}?tagType="+tagType
            });
        }
        ...
        function editTag() {
            ...
            showDialog({
                ...
                height: 300,
                title: "编辑标签",
                url: "${createLink(controller: 'parameterTag',action:'editTag')}?id=" + rows[0].id
            });
        }
        ...
        function confirmDeleteTag(ids){
            $("#deleteIds").val(ids);
            var url = "${createLink(controller: 'parameterTag',action:'delTagSubmit')}";
...
<e:layout fit="true">
    ...
    <e:form id="editForm">
        ...
        <div class="tright">
            ...
            <input type="hidden" name="submitType" id="submitType"/>
            <a href="javascript:void(0)" class="btn_orange" onclick="addTag()">添加标签</a>
            <a href="javascript:void(0)" class="btn_orange" onclick="editTag()">编辑标签</a>
            <a href="javascript:void(0)" class="btn_orange" onclick="delTag()">删除标签</a>
    ...
    <div id="gameDetailDlg" class="easyui-dialog" style="padding: 10px 20px" data-options="resizable:true,closed:true">
    </div>
